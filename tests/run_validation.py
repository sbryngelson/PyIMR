...
def solve_radius(times, R0, Req, material, **options):
    config = imr_fast.SimulationConfig(
        R0=R0, Req=Req, material=material, **options
    )
...
_checks = [("Zener truth De=2 s=6",
            solve_radius(
                _t2, _R0, _R0/6,
                imr_fast.Zener(2500., .1, 2*_t0, .4*_t0),
            ), _A[:,0])]
...
for lab, kw, ref in [
    ("qKV alphax=0.10", dict(material=imr_fast.QuadraticKelvinVoigt(2500., .1, .10)), "ref_qkv_a010.csv"),
    ("qKV alphax=0.25", dict(material=imr_fast.QuadraticKelvinVoigt(2500., .1, .25)), "ref_qkv_a025.csv"),
    ("UCM/OldB De=0.5", dict(material=imr_fast.OldroydB(.1, .5*_t0, .1*_t0)), "ref_ucm_De005.csv"),
    ("UCM/OldB De=2.0", dict(material=imr_fast.OldroydB(.1, 2*_t0, .4*_t0)), "ref_ucm_De020.csv"),
    ("Keller-Miksis NHKV", dict(material=imr_fast.NeoHookeanKelvinVoigt(2500., .1), radial=2), "ref_km_nhkv.csv"),
    ("Keller-Miksis Zener", dict(material=imr_fast.Zener(2500., .1, 2*_t0, .4*_t0), radial=2), "ref_km_zener.csv"),
    ...
    ("quadratic Zener", dict(material=imr_fast.QuadraticZener(2500., .1, 2*_t0, .4*_t0, .25)), "ref_stress4.csv"),
    ("radial=3 (KM enthalpy, Tait)",     dict(radial=3),                                        "ref_radial3.csv"),
    ("radial=4 (Gilmore, Tait)",         dict(radial=4),                                        "ref_radial4.csv"),
    ("radial=3+Zener", dict(radial=3, material=imr_fast.Zener(2500., .1, 2*_t0, .4*_t0)), "ref_radial3_zener.csv"),
    ("radial=4+Zener", dict(radial=4, material=imr_fast.Zener(2500., .1, 2*_t0, .4*_t0)), "ref_radial4_zener.csv"),
    ("radial=5 (KM enthalpy, Mie-Gruneisen)", dict(radial=5),                          "ref_radial5.csv"),
]:
    ml = np.loadtxt(f"{_d}/{ref}")
    material = kw.pop(
        "material", imr_fast.NeoHookeanKelvinVoigt(2500., .1)
    )
...
def _instantaneous_values(material, radius=.5, velocity=-.3, need_rate=True):
...
_elastic_reference = _instantaneous_values(
    imr_fast.InstantaneousMaterial(elastic=imr_fast.NeoHookean(2500.0))
)[0]
for _name, _elastic, _tol in [
    ...
    _value = _instantaneous_values(
        imr_fast.InstantaneousMaterial(elastic=_elastic)
    )[0]
    _rel = abs(_value-_elastic_reference)/abs(_elastic_reference)
    _ok = _rel < _tol; fail += (not _ok)
    print(f"    {_name:16s} rel={_rel:.2e}  {'PASS' if _ok else 'FAIL'}")

print("  viscous model reductions to Newtonian")
_viscous_reference = _instantaneous_values(
    imr_fast.InstantaneousMaterial(viscous=imr_fast.Newtonian(0.1))
)[0]
for _name, _viscous in [
    ...
    _value = _instantaneous_values(
        imr_fast.InstantaneousMaterial(viscous=_viscous)
    )[0]
    _rel = abs(_value-_viscous_reference)/abs(_viscous_reference)
    _ok = _rel < 1e-12; fail += (not _ok)
    print(f"    {_name:16s} rel={_rel:.2e}  {'PASS' if _ok else 'FAIL'}")

print("  analytic stress rates vs centered differences")
_materials = [
    imr_fast.InstantaneousMaterial(elastic=imr_fast.MooneyRivlin(1000., 400.)),
    imr_fast.InstantaneousMaterial(elastic=imr_fast.Yeoh(1000., 100., 10.)),
    imr_fast.InstantaneousMaterial(elastic=imr_fast.Fung(2500., .2)),
    imr_fast.InstantaneousMaterial(elastic=imr_fast.Gent(2500., 500.)),
    imr_fast.InstantaneousMaterial(elastic=imr_fast.ArrudaBoyce(2500., 50.)),
    imr_fast.InstantaneousMaterial(viscous=imr_fast.PowerLaw(.1, .7)),
    imr_fast.InstantaneousMaterial(
        viscous=imr_fast.CarreauYasuda(.1, .01, 1e-5, 2., .5)
    ),
    imr_fast.InstantaneousMaterial(
        viscous=imr_fast.Cross(.1, .01, 1e-5, 2.)
    ),
    imr_fast.InstantaneousMaterial(
        viscous=imr_fast.HerschelBulkley(100., .1, .8)
    ),
    imr_fast.InstantaneousMaterial(viscous=imr_fast.Bingham(100., .1)),
]
_rate_errors = []
for _material in _materials:
    _R, _Rd, _Rdd, _h = .5, -.3, .2, 1e-6
    _S, _Sdot, _, _A = _instantaneous_values(_material, _R, _Rd)
    _plus = _instantaneous_values(
        _material, _R + _h*_Rd, _Rd + _h*_Rdd, False
    )[0]
    _minus = _instantaneous_values(
        _material, _R - _h*_Rd, _Rd - _h*_Rdd, False
    )[0]
    _finite_difference = (_plus-_minus)/(2*_h)
    _predicted = _Sdot - _A/_R*_Rdd
    _rate_errors.append(
        abs(_finite_difference-_predicted)/max(1.0, abs(_finite_difference))
    )
_mx = max(_rate_errors)
_ok = _mx < 2e-7; fail += (not _ok)
...
try:
    solve_radius(
        _t[:3], _R0, _R0/6,
        imr_fast.InstantaneousMaterial(elastic=imr_fast.Gent(2500., 5.)),
    )
    _ok = False
...
_ucm_material = imr_fast.OldroydB(.1, _relaxation, _retardation)
_ucm = solve_radius(_tv, 225e-6, 225e-6/6, _ucm_material)
print("  zero nonlinearity must reproduce UCM/Oldroyd-B")
for _name, _model in [
    ("giesekus", imr_fast.Giesekus(.1, _relaxation, _retardation)),
    ("linear PTT", imr_fast.LinearPTT(.1, _relaxation, _retardation)),
]:
...
_ucm_km = solve_radius(
    _tv, 225e-6, 225e-6/6, _ucm_material, radial=2,
)
_distributed_km = solve_radius(
    _tv, 225e-6, 225e-6/6,
    imr_fast.Giesekus(.1, _relaxation, _retardation), radial=2,
)
...
_ucm_coupled = solve_radius(
    _tv, 225e-6, 225e-6/6, _ucm_material, **_coupled_options,
)
_distributed_coupled = solve_radius(
    _tv, 225e-6, 225e-6/6,
    imr_fast.Giesekus(.1, _relaxation, _retardation),
    **_coupled_options,
)
...
for _name, _model in [
    (
        "giesekus",
        imr_fast.Giesekus(.1, _relaxation, _retardation, mobility=0.2),
    ),
    (
        "linear PTT",
        imr_fast.LinearPTT(.1, _relaxation, _retardation, extensibility=0.2),
    ),
]:
    _R = solve_radius(_tv, 225e-6, 225e-6/6, _model)
