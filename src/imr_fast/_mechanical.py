...
def _forcing(time, p):
  ...
  amplitude = p[P_EE]
  if amplitude == 0.0: return 0.0j, 0.0j
  omega = p[P_OM]
  width = p[P_TW]
  delay = p[P_DT]
  exponent = p[P_MN]
  if wave_type == 0: return amplitude, 0.0j
  if wave_type == 1:
  ...
  if wave_type == 2:
    ...
    if time.real < lower or time.real > upper: return 0.0j, 0.0j
    cosine = 0.5 + 0.5 * np.cos(omega * (time - delay))
    return (amplitude * cosine**exponent, -amplitude * exponent * cosine ** (exponent - 1.0) * 0.5 * omega * np.sin(omega * (time - delay)))
...
def _elastic_integrand(code, parameters, stretch, pressure_scale):
  ...
  if code == 2: return -4.0 * parameters[0] / pressure_scale * geometric - 4.0 * parameters[1] / pressure_scale * (1.0 + stretch**-3)
  if code == 3:
    coefficient = 2.0 * (parameters[0] + 2.0 * parameters[1] * invariant_offset + 3.0 * parameters[2] * invariant_offset**2) / pressure_scale
  elif code == 4:
...
def _viscosity(code, parameters, shear_rate):
  ...
  if code == 3:
    ...
    if abs(shear_rate) < 1e-25: return zero, zero
  ...
  tangent = (
    yield_stress / regularization * np.exp(-scaled) + power_viscosity + consistency * (exponent - 1.0) * shear_rate**2 * effective ** (exponent - 3.0)
  )
  return viscosity, tangent

@njit(cache=True)
def _instantaneous_stress(elastic_code, elastic_parameters, viscous_code, viscous_parameters, nodes, weights, p, radius, velocity, need_rate):
  stress = 0.0j
  explicit_rate = 0.0j
  acceleration = 0.0j
  if elastic_code:
    ...
    if need_rate: explicit_rate += _elastic_integrand(elastic_code, elastic_parameters, wall_stretch, p[P_P8]) * velocity / p[P_REQ]
  if viscous_code:
...
def _stress(
  ...
  if material_code == 0: return 0.0j, 0.0j, 0.0j, rates
  if material_code == 1:
  ...
  if material_code == 3 or material_code == 4:
    ...
    rates[0] = -(z1 - equilibrium) / relaxation + 4.0 * (retardation - 1.0) / (reynolds * relaxation) * radius**2 * velocity
    stress_rate = rates[0] / radius**3 - 3.0 * velocity / radius**4 * z1 + 4.0 * retardation / reynolds * (velocity / radius) ** 2
    return stress, stress_rate, 4.0 / reynolds, rates
  if material_code == 5:
    z1, z2 = state[0], state[1]
    rates[0] = -(1.0 / relaxation - 2.0 * velocity / radius) * z1 + 2.0 * (retardation - 1.0) / (reynolds * relaxation) * radius**2 * velocity
    rates[1] = -(1.0 / relaxation + velocity / radius) * z2 + 2.0 * (retardation - 1.0) / (reynolds * relaxation) * radius**2 * velocity
    stress = (z1 + z2) / radius**3 - 4.0 * retardation / reynolds * velocity / radius
    stress_rate = (rates[0] + rates[1]) / radius**3 - 3.0 * velocity / radius**4 * (z1 + z2) + 4.0 * retardation / reynolds * (velocity / radius) ** 2
  ...
  if material_code == 7 or material_code == 8:
    ...
    for index in range(points):
      ...
      if material_code == 7:
        ...
        radial_rate = (
          -trace_factor * radial_stress / relaxation - 4.0 * local_rate * radial_stress - 4.0 * polymer_viscosity * local_rate / relaxation
        )
        hoop_rate = -trace_factor * hoop_stress / relaxation + 2.0 * local_rate * hoop_stress + 2.0 * polymer_viscosity * local_rate / relaxation
      ...
      integrand_rate[index] = 2.0 * ((radial_rate - hoop_rate) / material_radius - difference * material_velocity[index] / material_radius**2)
    ...
    if stress_weights.size == points:
      ...
      for index in range(points):
        ...
        if need_rate:
          polymer_rate += (
            ...
            * 2.0
            * ((rates[index] - rates[index + points]) * inverse_cubed - 3.0 * difference * radius**2 * velocity * inverse_cubed**2)
          )
    else:
      for index in range(points - 1):
        ...
        if need_rate:
          polymer_rate += 0.5 * (
            (integrand_rate[index] + integrand_rate[index + 1]) * interval + (integrand[index] + integrand[index + 1]) * interval_rate
          )
