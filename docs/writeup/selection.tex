...
\end{equation}
%
The replication half is cheap. The support half is a cardinality constraint and is not convex,
so it cannot join the objective without forfeiting \eqref{eq:gaincert}; measured on the operator
design, forcing three replicates leaves the batch at two settings and $\mathrm{df} = -2$ still.
What rescues it in practice is asking more than one question: the portfolio optimum below
spreads over six geometries on its own, and \eqref{eq:constrained} never binds.
