...
class RadiusObservation:
    ...
    def __post_init__(self):
        ...
        if deviation.ndim > 1 or (
            deviation.ndim == 1 and deviation.shape != time.shape
        ):
            raise ValueError(
                "standard_deviation_m must be scalar or match time_s"
            )
        if (
        ...
        if time[0] < 0.0 or np.any(np.diff(time) <= 0.0):
            raise ValueError(
                "observation time_s must be non-negative and increasing"
            )
        ...
        object.__setattr__(
            self, "standard_deviation_m", _readonly(deviation)
        )
...
class PreparedInference:
    ...
    def __post_init__(self):
        ...
        if not parameters or not all(
            isinstance(parameter, InferenceParameter)
            for parameter in parameters
        ):
            raise TypeError(
                "parameters must contain at least one InferenceParameter"
            )
        ...
        for parameter in parameters:
            ...
            if not np.isscalar(value) or not np.isfinite(value):
                raise ValueError(
                    f"{parameter.path!r} must identify a finite scalar field"
                )
            _normalize_parameters(
                self.config, (SensitivityParameter(parameter.path),)
            )
    ...
    def physical_parameters(self, unit_parameters):
        ...
        return np.array(
            [
                parameter.physical_value(value)
                for parameter, value in zip(
                    self.parameters, unit, strict=True
                )
            ]
        )
    ...
    def config_from_unit(self, unit_parameters):
        ...
        config = self.config
        for parameter, value in zip(
            self.parameters, physical, strict=True
        ):
            config = _replace_path(
                config, _path_parts(parameter.path), value
            )
        return config
    ...
    def residual(self, unit_parameters):
        ...
        return (
            result.radius_m - self.observation.radius_m
        ) / self.observation.standard_deviation_m
    ...
    def jacobian(self, unit_parameters):
        ...
        return (
            result.radius_m
            / self.observation.standard_deviation_m[:, None]
            * chain
        )
    ...
    def evaluate(self, unit_parameters):
        ...
        normalization = np.log(
            2.0
            * np.pi
            * self.observation.standard_deviation_m ** 2
        )
    ...
    def evaluate_batch(self, unit_parameters, workers=1):
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            return tuple(
                executor.map(_evaluate_worker, repeat(self), points)
            )
    ...
    def fit_multistart(
        ...
        *,
        ...
        start_points[0] = 0.5
        arguments = [
            (self, point, max_evaluations)
            for point in start_points
        ]
        if workers == 1:
            endpoints = tuple(
                _fit_worker(argument) for argument in arguments
            )
        else:
            with ProcessPoolExecutor(max_workers=workers) as executor:
    ...
    def _validate_unit_parameters(self, unit_parameters):
        ...
        if unit.shape != (self.size,):
            raise ValueError(
                f"unit parameters must have shape ({self.size},)"
            )
        if not np.all(np.isfinite(unit)) or np.any(
            (unit < 0.0) | (unit > 1.0)
        ):
    ...
    def _validate_batch(self, unit_parameters):
        ...
        if points.ndim != 2 or points.shape[1] != self.size:
            raise ValueError(
                f"batch parameters must have shape (n, {self.size})"
            )
        if not np.all(np.isfinite(points)) or np.any(
...
def _fit_worker(argument):
    ...
    try:
        ...
        return MultistartEndpoint(
            ...
            physical_parameters=_readonly(
                inference.physical_parameters(start)
            ),
            cost=float("inf"),
