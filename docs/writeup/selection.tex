...
That formulation optimizes the wrong variable. Pricing the candidates -- a trace of $N$ samples
costs $N$ frames -- puts the split between bubbles and frames inside the certified problem, and
it is worth $0.9$ to $10$ nats against at most $1.3$ for the geometry everything else optimizes.
Two questions the design chapter left open close on the same terms: the screening threshold,
which a maximin over the correlated-likelihood deflation removes, and the value of staging a
budget, which is nil because one bubble at a geometry chosen for it settles every live operator
rival. The temperature dependence closes nothing -- with temperature held it is structurally
unidentifiable, at any budget and any geometry.
\end{abstract}
...
\section{One criterion, and what a budget buys}
\label{sec:gain}
...
\subsection*{Will it settle? A budget in bubbles}
...
\subsection*{What the criterion cannot reach}
...
\subsection*{Making the discrepancy a coordinate}
