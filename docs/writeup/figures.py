...
def records():
  ...
  fig, axes = plt.subplots(1, 3, figsize=style.size(1.0, 2.1), sharey=True)
...
def fits():
  ...
  fig, ax = plt.subplots(figsize=style.size(1.0, 2.7))
  width = 0.27
  ...
  ax.text(len(names) - 0.4, 1.05, "measurement noise", ha="right")
  ax.set_yscale("log")
...
def trend():
  ...
  fig, ax = plt.subplots(figsize=style.size(0.5, 2.5))
  temps = [15, 23, 33]
