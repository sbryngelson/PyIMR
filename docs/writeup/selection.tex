...
likelihood rather than by anything about the models. That promotes the correlated likelihood
from a caveat on the magnitudes to a precondition for the design work: until it is fixed, a
batch aimed at a live pair may be aimed at a pair whose liveness is an artifact.
...
This is deliberately not folded into the geometry optimization. Doing so needs $\Sigma$ at a
geometry nobody has run, and three measurements of it against a twenty-four-fold span in $t_c$
is a weaker extrapolation than the noise-profile transfer of \cref{sec:noisedesign}, which was
at least tested.
