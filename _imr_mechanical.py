...
def _elastic_integrand(code, parameters, stretch, pressure_scale):
    ...
    if code == 2:
        return (
            -4.0 * parameters[0] / pressure_scale * geometric
            - 4.0
            * parameters[1]
            / pressure_scale
            * (1.0 + stretch ** -3)
    ...
    if code == 3:
        coefficient = (
            ...
            * (
                parameters[0]
                + 2.0 * parameters[1] * invariant_offset
                + 3.0 * parameters[2] * invariant_offset ** 2
            )
            / pressure_scale
        )
    elif code == 4:
        coefficient = (
            parameters[0]
            / pressure_scale
            * np.exp(parameters[1] * invariant_offset)
        )
    elif code == 5:
        coefficient = (
            parameters[0]
            / pressure_scale
            / (1.0 - invariant_offset / parameters[1])
        )
    else:
...
def _instantaneous_stress(
    ...
    if viscous_code:
        ...
        for index in range(nodes.size):
            ...
            viscosity, tangent = _viscosity(
                viscous_code, viscous_parameters, shear_rate
            )
...
def _stress(
    ...
    if material_code == 1:
        stress = (
            -(5.0 - 4.0 * ratio - ratio ** 4) / (2.0 * cauchy)
        ...
        stress_rate = (
            ...
            * (ratio + ratio ** 4)
            / cauchy
            + 4.0 / reynolds * (velocity / radius) ** 2
        )
        return stress, stress_rate, 4.0 / reynolds, rates
    if material_code == 2:
        stress = (
            (3.0 * stiffening - 1.0)
            * (5.0 - ratio ** 4 - 4.0 * ratio)
            / (2.0 * cauchy)
            - 4.0 / reynolds * velocity / radius
            + 2.0
            * stiffening
    ...
    if material_code == 3 or material_code == 4:
        ...
        stress = (
            z1 / radius ** 3
            - 4.0 * retardation / reynolds * velocity / radius
        )
        if material_code == 3:
            equilibrium = (
                -0.5
    ...
    if material_code == 7 or material_code == 8:
        ...
        for index in range(points):
            radius_cubed = (
                reference_radius_cubed[index] + radius ** 3 - 1.0
            )
            ...
            material_velocity[index] = (
                radius ** 2 * velocity / material_radius ** 2
            )
            integrand_rate[index] = 2.0 * (
                (radial_rate - hoop_rate) / material_radius
                - difference
                * material_velocity[index]
                / material_radius ** 2
            )
        ...
        for index in range(points - 1):
            ...
            interval_rate = (
                material_velocity[index + 1] - material_velocity[index]
            )
            polymer_integral += (
                0.5
            ...
            if need_rate:
                polymer_rate += 0.5 * (
                    (
                        integrand_rate[index]
                        + integrand_rate[index + 1]
                    )
                    * interval
                    + (integrand[index] + integrand[index + 1])
                    * interval_rate
        ...
        stress_rate = (
            polymer_rate + solvent * (velocity / radius) ** 2
        )
...
def _mie_antiderivative(mu, slope, nog):
    ...
    return (
        (2.0 * nog + slope - 1.0)
        / (slope + 1.0) ** 3
        * np.log(shifted / (mu + 1.0))
        + (nog + slope)
        / (slope * (slope + 1.0) * shifted ** 2)
        - (2.0 * nog + slope - 1.0)
        / ((slope + 1.0) ** 2 * shifted)
    )
...
def mechanical_rhs(
    ...
    if radial == 1:
        ...
        density_factor = (
            p[P_TAIT_SAM] / bubble_pressure
        ) ** (1.0 / p[P_TAIT_EXPONENT])
        enthalpy = (
            p[P_TAIT_SAM]
            / p[P_TAIT_NO]
            * (
                (bubble_pressure / p[P_TAIT_SAM]) ** p[P_TAIT_NO]
                - 1.0
            )
        )
        sound = p[P_CSTAR]
        if radial == 4:
            density = (
                bubble_pressure / p[P_TAIT_SAM]
            ) ** (1.0 / p[P_TAIT_EXPONENT])
            sound = np.sqrt(
                p[P_TAIT_EXPONENT] * bubble_pressure / density
            )
        numerator = (
        ...
        denominator = (
            (1.0 - velocity / sound) * radius
            + acceleration * density_factor / sound
        )
        radial_acceleration = numerator / denominator
    else:
        sound = p[P_CSTAR]
        ...
        enthalpy = sound ** 2 * (
            _mie_antiderivative(mu, p[P_HUGONIOT], p[P_NOG])
            - p[P_MIE_REFERENCE]
        )
        numerator = (
            (1.0 + velocity / sound) * (enthalpy - forcing)
...
def mechanical_tangent_rhs(
    ...
    """Evaluate the physical-time value and all tangent directions at once."""
    ...
    for direction in range(width):
        complex_parameters = (
            base_parameters
            + 1j * step * parameter_tangents[:, direction]
        )
        complex_state = (
            base_state + 1j * step * matrix[:, direction + 1]
        )
        complex_elastic = (
            base_elastic + 1j * step * elastic_tangents[:, direction]
        )
        complex_viscous = (
            base_viscous + 1j * step * viscous_tangents[:, direction]
        )
        nondimensional_time = time_s / complex_parameters[P_T0]
        output = mechanical_rhs(
            nondimensional_time,
            complex_state,
            complex_parameters,
            material_code,
...
def mechanical_stress_tangent(
    ...
    """Stress value and parameter tangents for one augmented output state."""
    width = matrix.shape[1] - 1
