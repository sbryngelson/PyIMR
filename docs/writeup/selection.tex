...
So \cref{sec:latent}'s mode is a clock error with two hands. A bubble has two independent
timescales, the collapse set by $R_{\max}$ and the ambient pressure and the afterbounce set by
the gas it is left with; the processing normalises by the first; and the mode is what the second
one leaves behind, which is why it lives after the collapse and why no parameter of any model
ever absorbed it. The remedy is a processing change, not a constitutive one.
