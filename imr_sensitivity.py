...
class SensitivityParameter:
    ...
    def __post_init__(self):
        ...
        if self.scale is not None and (
            not np.isfinite(self.scale) or self.scale <= 0.0
        ):
...
def _normalize_parameters(config, parameters):
    normalized = tuple(
        parameter
        if isinstance(parameter, SensitivityParameter)
        else SensitivityParameter(parameter)
    ...
    for parameter in normalized:
        ...
        if any(part in _NONDIFFERENTIABLE_FIELDS for part in parts):
            raise ValueError(
                f"{parameter.path!r} is discrete or controls solver preparation"
            )
        value = _path_value(config, parts, parameter.path)
        if not isinstance(value, Real) or not np.isfinite(value):
            raise ValueError(
                f"{parameter.path!r} must identify one finite scalar field"
            )
...
def _dual_config(config, normalized, values, scales):
    ...
    result = config
    for index, (parameter, value, scale) in enumerate(
        zip(normalized, values, scales, strict=True)
    ):
...
def _dual_parameters(config):
    ...
    if config.initial.internal_pressure_pa is not None:
        parameters["Pb"] = (
            config.initial.internal_pressure_pa / parameters["P8"]
        )
    return parameters
...
def _initial_matrix(problem, config, parameters, width):
    ...
    if problem.collapse_stats is not None:
        collapse_tangents = _collapse_initial_tangents(
            problem, config, width
        )
...
def _collapse_initial_tangents(problem, config, width):
    """Differentiate the shooting root and maximum-radius event implicitly."""
    ...
    if not solution.success or solution.t_events[0].size == 0:
        raise _solver.SimulationError(
            "collapse sensitivity precursor failed to reach maximum radius"
        )
    event = solution.y_events[0][-1].reshape(
    ...
    memory_tangents = (
        event[2:, 1:]
        + event_rhs[2:, [0]] * event_time_tangents
    )
    ...
    if abs(radius_velocity) < 1e-14:
        raise _solver.SimulationError(
            "collapse shooting root has a singular velocity derivative"
        )
    velocity_tangents = (
        -radius_tangents[:width] / radius_velocity
    )
    return (
        memory_tangents[:, :width]
        + memory_tangents[:, [velocity_direction]]
        * velocity_tangents
    )
...
def _dual_medium(problem, parameters):
    ...
    replacements = {
        ...
        "grad_Trans": (
            -coefficients * parameters["chi"] / delta_bubble
        ),
        "grad_C": (
            -coefficients
            * parameters["Fom"]
            * parameters["L_heat_star"]
            / delta_bubble
        ),
    }
...
def _material_parameters(material, width):
    ...
    elastic_values, elastic_tangents = _packed_values(
        elastic_fields, width, 5
    )
    viscous_values, viscous_tangents = _packed_values(
        viscous_fields, width, 5
    )
    return (
        material_code,
        elastic_code,
...
def _rhs_mechanical_compiled(
    ...
    *,
    ...
    nodes = (
        prepared.interval_nodes
        if prepared is not None
        else np.empty(0)
    )
    weights = (
        prepared.interval_weights
        if prepared is not None
        else np.empty(0)
    )
    distributed = problem.distributed_stress
    reference_radius = (
        distributed.reference_radius
        if distributed is not None
        else np.empty(0)
    )
    reference_radius_cubed = (
        distributed.reference_radius_cubed
        if distributed is not None
        else np.empty(0)
    )
...
def _compiled_mechanical_outputs(
    ...
    for time_index, row in enumerate(states):
        ...
        pressure = (
            (parameters["Pb"] - parameters["Pv"])
            * radius ** (-3.0 * parameters["kappa"])
            + parameters["Pv"]
        )
...
def _output_duals(
    ...
    bubble_temperature = (
        np.empty((count, config.Nt, width)) if config.bubtherm else None
    )
    medium_temperature = (
        np.empty((count, config.Mt, width)) if config.medtherm else None
    )
    vapor_fraction = (
        np.empty((count, config.Nt, width)) if config.masstrans else None
    )
    wall_state = _solver._WallState()
    ...
    for time_index, row in enumerate(states):
        ...
        stress_state = (
            dual_state[problem.layout.stress]
            if problem.layout.stress.stop > problem.layout.stress.start
            else None
        )
        ...
        for target, value in zip(targets, output, strict=True):
            target[time_index] = (
                value.tangent if isinstance(value, Dual) else np.zeros(width)
            )
        if config.bubtherm:
            theta = dual_state[problem.layout.bubble_thermal].copy()
            medium_state = (
                dual_state[problem.layout.medium_thermal].copy()
                if config.medtherm
                else None
            )
            vapor_state = (
                dual_state[problem.layout.vapor_fraction].copy()
                if config.masstrans
                else None
            )
            ...
            bubble_temperature[time_index] = _tangent_values(
                config.T8 * temperature, width
            )
            if medium_temperature is not None:
                medium_temperature[time_index] = _tangent_values(
                    config.T8 * medium_state, width
                )
            if vapor_fraction is not None:
                vapor_fraction[time_index] = _tangent_values(
                    vapor_state, width
                )
    return (
        radius_ratio,
...
def solve_with_sensitivities(problem, tv, parameters):
    ...
    width = len(normalized)
    initial = _initial_matrix(
        problem, dual_config, dual_parameters, width
    )

    started = perf_counter()
    use_compiled_mechanical = (
        not config.bubtherm
        and problem.forcing is None
    )
    if use_compiled_mechanical:
        parameter_values, parameter_tangents = _mechanical_parameters(
            dual_parameters, width
        )
    ...
    jacobian_sparsity = _augmented_sparsity(
        problem.jacobian_sparsity, width
    )
    ...
    compiled_output = (
        (parameter_values, parameter_tangents, material_data)
        if use_compiled_mechanical
        else None
    )
    ...
    outputs = tuple(
        None if output is None else output / scales
    ...
    return SensitivityResult(
        ...
        bubble_temperature_k=(
            None if outputs[5] is None else _readonly(outputs[5])
        ),
        medium_temperature_k=(
            None if outputs[6] is None else _readonly(outputs[6])
        ),
        vapor_mass_fraction=(
            None if outputs[7] is None else _readonly(outputs[7])
        ),
    )
