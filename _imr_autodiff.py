...
class Dual:
    """A scalar value with a dense vector of first derivatives."""
    ...
    def _binary(self, other, value, left, right):
        ...
        return Dual(
            ...
            left(self.value, other.value) * self.tangent
            + right(self.value, other.value) * other.tangent,
        )
    ...
    def __add__(self, other):
        ...
        return self._binary(other, lambda a, b: a + b, lambda _a, _b: 1.0,
                            lambda _a, _b: 1.0)
    ...
    def __mul__(self, other):
        if isinstance(other, np.ndarray):
            return self._array_operation(other, lambda value: self * value)
        return self._binary(other, lambda a, b: a * b, lambda _a, b: b,
                            lambda a, _b: a)
    ...
    def __pow__(self, other):
        ...
        if not np.any(other.tangent):
            tangent = (
                other.value * self.value ** (other.value - 1.0) * self.tangent
            )
            return Dual(result, tangent)
        if self.value <= 0.0:
            raise ValueError(
                "a differentiated exponent requires a positive base"
            )
        tangent = result * (
            other.tangent * np.log(self.value)
            + other.value * self.tangent / self.value
        )
        return Dual(result, tangent)
...
def primal(value):
    """Return scalar primal values while preserving ordinary inputs."""
    return value.value if isinstance(value, Dual) else value


def primal_array(values):
    """Extract a floating array from an arbitrary scalar/object array."""
...
def unpack(values, width):
    """Return primal values and a ``(n_values, width)`` tangent matrix."""
...
def seed(value, width, index=None, scale=1.0):
    """Create a constant or one-direction seed."""
