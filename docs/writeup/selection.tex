...
result~\citep{kiefer1960equivalence}. Judged that way, the experiments analyzed here are close
...
grid quadrature over that box, with the noise scale marginalized under a half-Cauchy
...
\emph{of the present design} optimizes for a direction that moves once the design changes,
...
$F$ is being computed. T-optimality asks instead for the design maximizing the discrepancy
...
\noindent Reported per axis rather than as one determinant, because a design maximizing
...
parameterizes the physics, not the information in the record.
...
\end{equation}
%
and the minimization is the whole content of it. A rival that can retune its parameters to
...
parameters, or a minimization that terminates early -- reports discrimination that is not
...
gave $0.67$. Restarting and polishing that minimization changed other entries again: at
...
$T$ is defined by an inner \emph{global} minimization, and that minimization is multimodal.
...
optimiser lowers the scores, because a better-optimized rival imitates better. The tabulated
...
coordinate -- an eigenvalue of $J^{\mathsf T}J$, with no inner optimization -- is not
...
this experiment favor the truth''.
...
reliability is carried by the spread of the weights, summarized by the effective sample size
%
\begin{equation}
...
-- leaves $H \simeq J^{\mathsf T}J$, already computed by the fit. Parameterizing so the prior
...
of greatest standardized prediction variance -- D-optimality and G-optimality coincide.
...
literature scalarizes with weighted sums where this study needed a Chebyshev form. The
non-convexity that forced the more elaborate scalarization was an artefact of optimizing over
...
would need of order $10^{10}$ points. Discretization error of $17.3$ sits well inside the
