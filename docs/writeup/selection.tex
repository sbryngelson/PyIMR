...
\title{Which constitutive model do cavitation records prefer,\\
and what experiment would settle it?}
...
\begin{abstract}
We ask two questions of laser-induced cavitation records in gelatin at three temperatures: which
constitutive model the data prefer, and what experiment would settle the question better. The
second turns out to be the harder one, and most of this paper is about it.

For the first, Bayesian model selection over seventeen candidates, with a noise scale measured
from trial-to-trial spread and a prior that prices a model down where it merely reproduces a
simpler one it contains. A quadratic Zener solid -- strain-stiffening elasticity \emph{and}
stress relaxation -- wins at every temperature, reaching $\chi^2/N = 1.23$ to $1.35$ on the
selection grid and $0.96$ refitted to its true optimum at \SI{15}{\celsius} --- figures that
overstate the agreement, for a reason given below and worth carrying from here. Neither ingredient
suffices alone: relaxation without stiffening reaches $1.98$, and both memoryless candidates land
at $20$, where they fit each other to one part in a thousand. The stiffening term does no work at
all without a relaxation mode to act on, and its advantage collapses as the gel warms -- the
clearest physical signal in the comparison, though one carried by three temperatures with $18$,
$14$ and $7$ trials, which makes it suggestive rather than established.

We report the limitations plainly. The residuals are ceilings rather than estimates, because the
winning model's best-fitting region is partly unintegrable, and a good $\chi^2/N$ overstates how
well the model does: the residual has lag-one autocorrelation $0.918$, so the $201$ samples carry
the information of about ten independent ones, and what remains is structure the model lacks
rather than noise it failed to average away. The ranking survives this -- a $3\%$ change under a
correlated likelihood -- but the absolute claim does not.

For the second, scoring by parameter precision and by model discrimination gives criteria that
disagree, and both prove delicate: a criterion defined by an inner minimum is multimodal here,
and every improvement to the optimizer lowers it. Reformulating over design \emph{measures}
rather than single designs makes the problem convex, and the Kiefer--Wolfowitz equivalence
theorem then certifies the answer -- over the candidate set, a qualifier that turns out to be
load-bearing and then repayable. On a grid eight times finer the sensitivity reaches $69.6$
against a bound of $4$, so the certified optimum is at first provably best only among the
settings it was offered. Refining the candidate set where the scan flags, and rescanning an
independent set each round, drives the excess to $0.017$ -- four tenths of a percent of the
bound, from a single point converging on one already in the support, and falling by about two
fifths per round. That is how discretisation behaves rather than an obstruction, though the
sequence was stopped at four rounds and not run to its limit.

What the certified problem is worth depends on which variable it optimizes, and the geometry
everything else optimizes is not the important one. Pricing the candidates -- a trace of $N$
samples costs $N$ frames -- puts the split between bubbles and frames inside the certified
problem, and that split is worth $+3.4$ to $+4.0$ nats against at most $1.34$ for the geometry
--- and it survives the discovery that a quarter of the between-trial scatter it rests on is a
clock error rather than a material one.
Two further corrections move the recommendation rather than its error bar: a geometry is not a
knob, and designing as though it were costs about a nat, on scatter widths confirmed here against
$437$ per-event radii; and averaging over a computed model-error bound rather than a point
estimate is worth $0.39$ to $0.67$. The temperature dependence closes nothing -- with temperature
held it is structurally unidentifiable, at any budget and any geometry.
\end{abstract}
...
modulus is a statement about the prior box -- and \cref{sec:discrepancy} shows this is not a
bound that can be widened away, since releasing one wall merely slides the fit onto another. The bubble-dynamics operator assumed throughout
...
On the design side the useful conclusion is about what to optimize rather than what the optimum
is. The certified formulation was applied to the geometry, which its own numbers rank last:
