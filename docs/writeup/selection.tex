...
\noindent Every record wants a larger equilibrium radius, by three to eleven percent against the
\SI{1.68}{\percent} this section opens by calling worth an operator change.

The direction of the evidence is that this is calibration and not the model helping itself to
one more parameter. $\chi^2$ alone cannot tell those apart --- any added freedom lowers it ---
but lack of fit can, because replicates supply pure error that costs no model. It falls on every
record, and falls proportionally FURTHER than $\chi^2$ does: by factors of $0.52$, $0.73$ and
$0.83$ against $0.65$, $0.88$ and $0.86$. Systematic misfit is genuinely reduced, not
redistributed.

Two things stop that from being a licence to overwrite the stored numbers, and both are reasons
to leave them and report this instead. The offsets are not one convention error --- they differ
by a factor of three across records, so nothing derivable explains them all. And lack of fit
still rejects after the correction, at $8.80$, $2.15$ and $1.49$, so the model remains wrong and
some part of what $R_{\rm eq}$ absorbed was its error rather than the apparatus's. Replacing a
recorded quantity with one inferred from the model under test would make every later fit partly
circular, and the honest resolution is to re-derive the stretch from the source measurement.
...
of \emph{five and a half} at \SI{15}{\celsius}. The relaxation times in this document are
conditional on a stretch that three records agree is too large.
