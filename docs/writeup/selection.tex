...
Three results follow. The likelihood depends on the product $g\alpha$ alone, so the shear
modulus is set by where the prior box cuts a ridge and not by the data; moving the box swings
the model ranking by fifty nats. The bubble-dynamics operator, held fixed at Keller--Miksis in
every published comparison we are aware of, is beaten on all three records by
Gilmore/Tait. And the correlated residual that motivates richer constitutive models is not
removed by a second relaxation time, by a rate-dependent one, by fitting individual trials, or
by any of six operators.

We also ask what experiment would settle the question. Treating the choice of model as one
more unknown to be fitted lets a single calculation say how well a given experiment separates
the models and how well it pins the material. The answer comes with a proof of optimality,
which is unusual for a design calculation and follows from a classical
result~\citep{kiefer1960equivalence}. Judged that way, the experiments analysed here are close
to the best possible for the hardest of the three choices and $24$ to $41$ times worse than
the best for the two easiest.
...
recorded as $R(t)/R_{\max}$. \Cref{fig:records} shows the mean trace and the
...
deviation understates uncertainty -- and is handled in \cref{sec:beta} by giving the noise
...
and \cref{sec:identifiability} shows this one is not identified in a direction. The
...
The c-optimal choice carries a smallr trap. Nominating $c$ as the sloppy direction
...
\Cref{fig:valley} shows why the four estimates in this study disagree about $g$ by an
...
\cref{sec:identifiability} and returns $\mathrm{sd}(\log g) = 159$ -- but only the
...
one is larger -- which separates two things that \cref{sec:limitations} had conflated.
...
marginalisation of \cref{sec:beta} absorbs the difference -- the implied $\beta \approx 0.24$
...
Keller--Miksis/Mie--Gr\"uneisen ranks first at $+4.02$ nats and Gilmore/Tait second to last at
...
ranking: Gilmore/Tait leads at $+21.93$ and KM/Mie--G falls to $+7.32$.
...
mechanism: the optimiser slides along the $g\alpha$ valley until it meets an edge.
...
constitutive law, the bubble-dynamics equation, and whether heat transfer is modelled.
Published comparisons vary the first and leave the other two at the origin. Some of each
change can be cancelled by refitting stiffness and viscosity, which makes it invisible no
...
reproduced by refitting the material and is invisible at every design (\cref{fig:absorption});
...
absorbed axis by a wide margin, which is the mechanism behind \cref{sec:operator}: most of an
...
The scaling argument of \cref{sec:criteria} says that a design change which merely
...
concave function of it is concave in the design, and the maximisation becomes convex. That
...
study rather than as a step toward a parameter value. Ranking by evidence in coordinates the
records can answer, three findings hold. The shear modulus is not identified separately from
the stiffening exponent, so a reported modulus is a statement about the prior box. The
bubble-dynamics operator assumed throughout this literature is beaten on every record. And a
correlated residual that has been read as constitutive model error survives every
constitutive, observational and operator change tested here.
...
\cref{sec:enrich} the uncapped form favoured the two-mode law by $29.7$ nats exactly where
...
reparameterisation or a fixed value helps. Right: the misfit is closed but very elongated, $F$
