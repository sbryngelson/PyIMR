...
\paragraph{A caution about nested criteria.}
$T$ is defined by an inner optimisation, so every reported value is an upper bound whose
tightness is unknown until it is checked. Enumerating the outer design space exactly does
not repair this: a dense grid over a criterion that is itself unreliable produces a front
that is precisely wrong. Any criterion containing an argmin --- $T$-optimality, profile
likelihoods, minimax designs --- needs its inner solve verified independently of the outer
search, and the cheapest sufficient check is that the criterion vary smoothly between
neighbouring designs, since the underlying physics does.
...
question that has no scalar answer. What replaces it is the set of designs on which no
other design is better on every criterion at once. Section~\ref{sec:tradeoff} constructs
it.
