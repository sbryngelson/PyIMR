...
flexible one. The evidence adds the Occam factor of Fig.~\ref{fig:occam}, which charges a
...
which is why the pathology in Fig.~\ref{fig:occam} matters.}
...
$54$ fits sits on a bound --- and makes the question well posed. Repeating at three ratios
spanning two decades and on all three records, an ordering counts as settled when some record
decides it by more than a nat and none contradicts it; records without the power to tell
...
\midrule
any compressible operator $>$ Rayleigh--Plesset & $+162$ to $+182$ & $+74$ to $+79$ & $+74$ to $+75$ \\
...
\noindent Twelve of the fifteen pairwise orderings are settled. Compressibility is required,
decisively and on every record. And \texttt{keller-miksis}, the pressure form every candidate
in this package assumes, is beaten by three separate operators --- by Gilmore/Tait on all
three records, and by both enthalpy Keller--Miksis forms on two.

\paragraph{An earlier version of this table was wrong, and how it was wrong is the point.}
Run with six multistart restarts rather than ten, the same comparison put Gilmore/Tait fourth
on two records and first on the third, and reported the operator as undetermined. It also
found no ordering at all at \SI{33}{\celsius}, which was read as a record without the power to
discriminate. Both were artefacts of the search. At ten starts the \SI{33}{\celsius} fits
reach $\chi^2/N = 0.42$ where six starts had found $1.151$: every operator had been stuck in
the same poor basin, which is indistinguishable from every operator fitting equally well. A
record that cannot decide and a search that did not converge look alike in the output, and
only a better search separates them.
...
\begin{tikzpicture}[scale=1.05]
  ...
  \node[red!75!black,anchor=south east] at (9.2,2.52) {\footnotesize the bound $p$};
  \node[blue!70!black,anchor=north west] at (0.2,1.05) {\footnotesize $d(x,\xi^\star)=\operatorname{tr}[M(\xi^\star)^{-1}M(x)]$};
  \node[red!75!black,anchor=south] at (1.5,3.0) {\footnotesize support: touches the bound};
  \draw[red!75!black,thin] (1.5,2.92) -- (1.5,2.52);
  \draw[red!75!black,thin] (2.6,2.92) -- (4.42,2.52);
  \draw[red!75!black,thin] (3.0,2.92) -- (7.38,2.52);
\end{tikzpicture}
...
Measured on the \SI{15}{\celsius} record and reported in \S\ref{sec:axes}: the bubble-dynamics
...
the data is flat along $g/\alpha$. An optimiser slides until it meets the edge of the box, and
...
in the operator ranking of \S\ref{sec:operator} when the bounds were widened, and behind the
order-of-magnitude disagreement in $g$ between methods in Fig.~\ref{fig:valley}.}
\label{fig:ridge}
\end{figure}
